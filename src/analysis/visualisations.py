...
def plot_pretraining(train_losses, val_losses, epochs, lr, batch_size, train_size, num_layers, save_dir="results"):
    ...
    plt.plot(train_losses, label='AMP-DA-Net Train Loss', linewidth=2)
    plt.plot(val_losses, label='AMP-DA-Net Val Loss', linewidth=2)
    plt.yscale('log')
    plt.xlabel("Epoch")
    plt.ylabel("AMP-DA-Net Loss")
    plt.title(f"AMP-DA-Net Loss Curves\nEpochs: {epochs}, LR: {lr}, Batch: {batch_size}, TrainSize: {train_size}, Layers: {num_layers}")
    plt.legend()
